    ...
    int K)
{
   ...
   /*
 * Two register buffers.
 *
 * Buffer 0 holds one reduction fragment.
 * Buffer 1 holds the next reduction fragment.
 */
     ...
     ++load)
{
    ...
    /*
     * Vectorized global-memory load.
     *
     * This remains safe when:
     *
     * - N is divisible by four
     * - a_col is a multiple of four
     * - all four elements are valid
     */
    if (a_row < M &&
        a_col + 3 < N &&
        N % VECTOR_WIDTH == 0)
    {
        ...
        /*
         * Do not use a float4 store here.
         *
         * Because shared_A has 17 floats per row,
         * many rows are not aligned to 16 bytes.
         */
        shared_A[shared_row][shared_col + 0] = value.x;
        shared_A[shared_row][shared_col + 1] = value.y;
        shared_A[shared_row][shared_col + 2] = value.z;
        shared_A[shared_row][shared_col + 3] = value.w;
    }
    else
    {
        /*
         * Boundary or unaligned path.
         */
        #pragma unroll
        for (int element = 0;
     ...
     ++load)
{
    ...
    else
    {
        ...
        /*
 * ---------------------------------------------------------
 * Preload reduction fragment 0
 * ---------------------------------------------------------
 *
 * Before computation begins, load the first eight A values
 * and eight B values into register buffer 0.
 */
#pragma unroll
for (int r = 0; r < MICRO_M; ++r)
{
    reg_A[0][r] =
        shared_A[local_row + r][0];
}
...
/*
 * Process the BLOCK_K reduction positions.
 */
#pragma unroll
for (int i = 0; i < BLOCK_K; ++i)
{
    /*
     * The current register buffer alternates:
     *
     * i = 0 -> buffer 0
     * i = 1 -> buffer 1
     * i = 2 -> buffer 0
     * i = 3 -> buffer 1
     */
    const int current_buffer = i & 1;
    const int next_buffer = current_buffer ^ 1;

    /*
     * Load the next reduction fragment before calculating the
     * current fragment.
     *
     * These loads do not depend on the current accumulator
     * calculations, giving the compiler an opportunity to
     * interleave loads and FMAs.
     */
    if (i + 1 < BLOCK_K)
    {
        #pragma unroll
        ...
        for (int c = 0; c < MICRO_N; ++c)
        {
            ...
            reg_B[next_buffer][c] =
                shared_B[i + 1][shared_col];
        }
    }

    /*
     * Calculate the 8 × 8 outer product using the current
     * register fragment.
     */
    #pragma unroll
    for (int r = 0; r < MICRO_M; ++r)
